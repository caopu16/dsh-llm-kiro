...
export { clearTokenCache, resolveToken } from './auth.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  const adapter = new KiroAdapter({
    ...
    resolveToken: (connection, signal) => resolveToken({
      ...
      // The token refresh shares the request's egress: an environment that can
      // only reach Kiro through the proxy cannot reach the OIDC endpoint directly.
